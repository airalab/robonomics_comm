...
class Matcher:
    ...
    def match(self, bid=None, ask=None):
        ...
        if not ask:
            h = hash((bid.model, bid.cost, bid.count))
            if h in self.bids:
                self.bids[h].append(bid)
            else:
                self.bids[h] = [bid]
        else:
            h = hash((ask.model, ask.cost, ask.count))
            if h in self.asks:
    ...
    def new_liability(self, ask, bid):
        ...
        '''
        exps = [ask.cost * ask.count, ask.lighthouseFee, bid.lighthouseFee, bid.validatorFee]
        sign = [ ask.salt
               ...
               , bid.signature[0:32]
               , bid.signature[32:64] ]
        self.builder.transact({'gas': 1000000}).create(b58decode(ask.model)[2:], b58decode(ask.objective)[2:], exps, sign)
