...
class ERC20Services:
    def __init__(self):
        ...
        def account_xrt_balance_handler(m):
            return AccountBalanceResponse(type_converter.strToUInt256(self.eth_utils.getTokenBalance(m.account.address)))
        rospy.Service('account_xrt_balance', AccountBalance, account_xrt_balance_handler)

        def xrt_balance_handler(m):
            return BalanceResponse(type_converter.strToUInt256(self.eth_utils.getTokenBalance(self.__account.address)))
        rospy.Service('xrt_balance', Balance, xrt_balance_handler)

        def account_to_address_xrt_allowance_handler(m):
        ...
        rospy.Service('account_xrt_allowance', AccountToAddressAllowance, account_to_address_xrt_allowance_handler)

        def xrt_allowance_handler(m):
        ...
        rospy.Service('xrt_allowance', Allowance, xrt_allowance_handler)
